...
==================================
Builds on Phase 4's overlay rendering and pipes the final composited frame
into a virtual camera device using pyvirtualcam, so it appears as a
selectable webcam in Google Meet, Messenger, Zoom, or any other app.

REQUIREMENTS (one-time setup):
  Windows: install OBS (https://obsproject.com) — OBS 26+ bundles the
           virtual camera driver pyvirtualcam uses. You do NOT need to
           keep OBS open or start its virtual camera manually on Windows.
  macOS:   install OBS, open it once, click "Start Virtual Camera" then
           "Stop Virtual Camera" (one-time registration step), close OBS.
  Linux:   sudo apt install v4l2loopback-dkms
           sudo modprobe v4l2loopback devices=1

Then in your call app (Meet/Messenger/etc.), open camera settings and
select "OBS Virtual Camera" instead of your real webcam.

Controls (local preview window):
  q       - quit
  o       - toggle overlay style: face-replacement <-> reaction bubble
  t       - toggle overlay on/off entirely
  p       - toggle local preview window on/off (virtual cam keeps running either way)
...
FADE_FRAMES = 8
BUBBLE_SIZE_RATIO = 0.28
FACE_BBOX_PADDING = 0.35
TARGET_FPS = 24  # virtual cam output framerate
...
def draw_hud(frame, features, raw_label, stable_label, fps, overlay_mode, overlay_on, cam_active):
    x, y = 10, 25
    line_height = 20
    cam_status = "VCAM: ON" if cam_active else "VCAM: OFF (pyvirtualcam not available)"
    cv2.putText(frame, f"FPS: {fps:.1f}  mode: {overlay_mode}  overlay: {'ON' if overlay_on else 'OFF'}  {cam_status}",
                (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1)
...
def main():
    ...
    if pyvirtualcam is None:
        ...
        print("(and set up OBS / v4l2loopback per the instructions at the top of this file)\n")
    ...
    if pyvirtualcam is not None:
        try:
            cam = pyvirtualcam.Camera(width=frame_w, height=frame_h, fps=TARGET_FPS)
            print(f"Virtual camera active: {cam.device}")
            print("Select this device as your camera in Meet/Messenger/etc.\n")
        except Exception as e:
            print(f"WARNING: Could not start virtual camera ({e})")
            print("Falling back to preview-only mode. Check your OBS/v4l2loopback setup.\n")
            cam = None
    ...
    while cap.isOpened():
        ...
        # --- Send clean frame (no HUD text) to virtual camera ---
        if cam is not None:
            cam_frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            cam.send(cam_frame_rgb)
            cam.sleep_until_next_frame()

        # --- HUD + local preview only, never sent to the virtual cam ---
        now = time.time()
        ...
        if show_preview:
            preview = frame.copy()
            cv2.putText(preview, status, (10, h - 40), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, status_color, 2)
            if last_features:
                draw_hud(preview, last_features, last_raw_label, last_stable_label,
                          fps, overlay_mode, overlay_on, cam is not None)
            cv2.imshow("Phase 5 - Virtual Camera (local preview)", preview)
        else:
            # still need waitKey to process window events even with no window shown
            if cv2.getWindowProperty("Phase 5 - Virtual Camera (local preview)", 0) >= 0:
                cv2.destroyWindow("Phase 5 - Virtual Camera (local preview)")
